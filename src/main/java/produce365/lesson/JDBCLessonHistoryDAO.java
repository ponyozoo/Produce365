...
public class JDBCLessonHistoryDAO implements LessonHistoryDAO {
	...
	public boolean insert(LessonHistory lessonHistory) {
		...
		try (Connection conn = DataSource.getDataSource();
				PreparedStatement pStatement = conn.prepareStatement(
						"INSERT INTO LESSON_HISTORY (LESSON_DATE, LESSON_ID, TRAINEE_ID) VALUES (?, ?, ?)")) {
	...
	public List<LessonHistory> selectByTraineeId(int id) {
		...
		try (Connection conn = DataSource.getDataSource();
				PreparedStatement pStatement = conn.prepareStatement(
						"SELECT H.IDX, H.LESSON_DATE, L.SUBJECT, T.NAME FROM LESSON_HISTORY H, LESSON L, TRAINEE T WHERE H.LESSON_ID = L.ID AND H.TRAINEE_ID = T.ID AND H.TRAINEE_ID = ? ORDER BY LESSON_DATE")) {
	...
	public List<LessonHistory> selectByLessonId(int id) {
		...
		try (Connection conn = DataSource.getDataSource();
				PreparedStatement pStatement = conn.prepareStatement(
						"SELECT H.IDX, H.LESSON_DATE, L.SUBJECT, T.NAME FROM LESSON_HISTORY H, LESSON L, TRAINEE T WHERE H.LESSON_ID = L.ID AND H.TRAINEE_ID = T.ID AND H.LESSON_ID = ? ORDER BY LESSON_DATE")) {
			pStatement.setInt(1, id);
			ResultSet queryResult = pStatement.executeQuery();
			result = new ArrayList<LessonHistory>();
			while (queryResult.next()) {
