...
public class JDBCCareHistoryDAO implements CareHistoryDAO {
	...
	public boolean insert(CareHistory careHistory) {
		...
		try (Connection connection = DataSource.getDataSource();
				PreparedStatement pStatement = connection.prepareStatement(
						"INSERT INTO CARE_HISTORY ( CARE_DATE, CARE_ID, TRAINEE_ID ) VALUES ( ?, ?, ? )")) {
	...
	public List<CareHistory> selectByTraineeId(int id) {
		...
		try (Connection connection = DataSource.getDataSource();
				PreparedStatement pStatement = connection.prepareStatement(
						"SELECT H.IDX, H.CARE_DATE, C.CATEGORY, T.NAME FROM CARE_HISTORY H, CARE C, TRAINEE T WHERE H.CARE_ID = C.ID AND H.TRAINEE_ID = T.ID AND H.TRAINEE_ID = ? ORDER BY CARE_DATE")) {
	...
	public List<CareHistory> selectByCareId(int id) {
		...
		try (Connection connection = DataSource.getDataSource();
				PreparedStatement pStatement = connection.prepareStatement(
						"SELECT H.IDX, H.CARE_DATE, C.CATEGORY, T.NAME FROM CARE_HISTORY H, CARE C, TRAINEE T WHERE H.CARE_ID = C.ID AND H.TRAINEE_ID = T.ID AND H.CARE_ID = ? ORDER BY CARE_DATE")) {
			pStatement.setInt(1, id);
			ResultSet rs = pStatement.executeQuery();
